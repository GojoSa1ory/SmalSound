...
export class TrackCardComponent{
  ...
  @Input() page: "playlist" | "favorite" | "userPublication" | "none" = "none"
  @Input() playlistId!: number
  @Input() isVerifided!: boolean
  // @Input() isPlaylistPage: boolean = false
  // @Input() isUserPublicationPage: boolean = false
  ...
  addTrackToFavorite () {
    this.favoriteService.addTrackToFavorite(this.track.id).subscribe({
      next: value => {
        this.notification.showNotification(false, "", "Избранное успешно обновлено")
        setTimeout(() => this.notification.closeNotification(), 2000)
      },
      error: err => {
        this.notification.showNotification(true, "Ошибка при изменении избранного")
        setTimeout(() => this.notification.closeNotification(), 2000)
      }
    })
  }
  ...
  removeFromPlaylist () {
   if(this.page === "userPublication") {
     this.trackService.deleteTrack(this.track.id)
   } else if (this.page === "playlist") {
     this.playlistService.deleteTrackFormPlaylist(this.playlistId, this.track.id).subscribe({
       next: (value: any) => {
         this.playlistService.playlist$.set(value.data)
         this.notification.showNotification(false, "", "Успешно")
         setTimeout(() => this.notification.closeNotification(), 2000)
       },
       error: err => {
